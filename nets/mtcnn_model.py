...
#define prelu
def prelu(inputs):
    alphas = tf.compat.v1.get_variable("alphas", shape=inputs.get_shape()[-1], dtype=tf.float32, initializer=tf.compat.v1.constant_initializer(0.25))
...
def cls_ohem(cls_prob, label):
    ...
    label_filter_invalid = tf.compat.v1.where(tf.less(label,0), zeros, label)
    ...
    valid_inds = tf.compat.v1.where(label < zeros,zeros,ones)
...
def bbox_ohem_smooth_L1_loss(bbox_pred,bbox_target,label):
    ...
    valid_inds = tf.compat.v1.where(label!=zeros_index,tf.ones_like(label,dtype=tf.float32),zeros_index)
    abs_error = tf.abs(bbox_pred-bbox_target)
    loss_smaller = 0.5*((abs_error*sigma)**2)
    loss_larger = abs_error-0.5/(sigma**2)
    smooth_loss = tf.reduce_sum(tf.compat.v1.where(abs_error<threshold,loss_smaller,loss_larger),axis=1)
...
def bbox_ohem(bbox_pred,bbox_target,label):
    ...
    valid_inds = tf.compat.v1.where(tf.equal(tf.abs(label), 1),ones_index,zeros_index)
    #(batch,)
...
def landmark_ohem(landmark_pred,landmark_target,label):
    ...
    valid_inds = tf.compat.v1.where(tf.equal(label,-2),ones,zeros)
...
def cal_accuracy(cls_prob,label):
    ...
    cond = tf.compat.v1.where(tf.greater_equal(label_int,0))
